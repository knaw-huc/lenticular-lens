...
@authenticated
def job_list():
    return jsonify(session['user'].list_jobs() if auth else get_all_jobs())
...
def job_create():
    ...
    if auth:
        session['user'].register_job(job_id, 'owner')
...
@with_job
def job_data(job):
    if auth and session['user'].user_id not in job.data['users']:
        session['user'].register_job(job.data['job_id'], 'shared')
