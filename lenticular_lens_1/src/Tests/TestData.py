...
def get_cluster_node(cluster_position = 1):
    from src.LLData.Serialisation import CLUSTER_SERIALISATION_DIR
    CLUSTERS = pickle_deserializer(serialised_folder=CLUSTER_SERIALISATION_DIR, name="{}.bin".format(
        "cluster"))
    ...
    if CLUSTERS:
        for key, data in CLUSTERS.items():
            ...
            count += 1
    return cluster
CLUTER = get_cluster_node(cluster_position=3)
