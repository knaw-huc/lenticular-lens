...
CREATE OR REPLACE FUNCTION public.jsonb_to_array(jsonb) RETURNS text[]
    IMMUTABLE AS
$$
BEGIN
...
CREATE OR REPLACE FUNCTION public.ecartico_full_name_list(jsonb) RETURNS jsonb
    IMMUTABLE AS
$$
DECLARE
...
CREATE OR REPLACE FUNCTION public.to_date_immutable(text) RETURNS date
    IMMUTABLE AS
$$
BEGIN
...
CREATE OR REPLACE FUNCTION public.get_year(text) RETURNS int
    IMMUTABLE AS
$$
DECLARE
    year text;
...
CREATE OR REPLACE FUNCTION public.get_month(text) RETURNS int
    IMMUTABLE AS
$$
DECLARE
    month text;
...
CREATE OR REPLACE FUNCTION public.get_year_month(text) RETURNS text
    IMMUTABLE AS
$$
DECLARE
    month int;
...
try:
    ...
    print("ERROR FOR COMPUTING EDIT DISTANCE: {}".format(str(err)))
    return 0
$$ LANGUAGE plpython3u;