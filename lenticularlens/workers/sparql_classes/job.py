...
class SPARQLClassesJob(WorkerJob):
    ...
    @staticmethod
    def get_classes_data(classes_data: list[dict[str, Value]], explicit_class_uri: str | None = None) -> list[ClassInfo]:
        classes = []
        for class_data in classes_data:
            class_uri = explicit_class_uri or class_data.get('class').value
            label = class_data.get('label').value if 'label' in class_data and class_data.get('label').value else None
