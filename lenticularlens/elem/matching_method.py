...
from lenticularlens.util.helpers import get_yaml_from_file, flatten
from lenticularlens.elem.matching_method_property import MatchingMethodProperty
        ...
        if self.is_list_match:
            ...
            new_similarity_template = cleandoc(f'''	
                jsonb_build_object('scores', ARRAY(
                    ...
                    FROM unnest({','.join([f'array_cat_agg({field_sql})' for field_sql in source_fields_sqls])}) AS src({','.join(source_alias_sqls)})
                    JOIN unnest({','.join([f'array_cat_agg({field_sql})' for field_sql in target_fields_sqls])}) AS trg({','.join(target_alias_sqls)})
                    ON {match_template}
